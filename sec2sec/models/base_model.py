...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, enc_hid_dim, dec_hid_dim, dropout, attention, emb_weights=None):
        super().__init__()
...
class Seq2Seq(nn.Module):
    ...
    def translate(self,src, beam_size = 5, max_len = 20, device = None):
        
        if device == None:
            device = 'cuda' if torch.cuda.is_available() else 'cpu'

        src = torch.LongTensor(src).unsqueeze(1).repeat(1, beam_size).to(device) 
        encoder_outputs, hidden = self.encoder(src)  #outputs [sent len, 1, enc hid dim * 2] #hidden = [1, dec hid dim]
        ...
        for i in range(max_len):
            dec_logits, hidden, att = self.decoder(output, hidden, src, encoder_outputs, mask)
            ...
            new_scores = scores.flatten()[order_scores][:beam_size].unsqueeze(1)
